...
#         assert self.monitor.current_status == PipelineStatus.HEALTHY
    
#     def test_health_check_structure(self):
#         result = self.monitor.check_pipeline_health()
...
#         # Mock the health checks to simulate failure
#         original_network_check = self.monitor._check_network_health
        
#         def mock_network_failure():
#             return False, "Simulated network failure"
        
#         self.monitor._check_network_health = mock_network_failure
        
#         result = self.monitor.check_pipeline_health()
        
#         # Restore original method
#         self.monitor._check_network_health = original_network_check
...
#         assert 'last_checked' in report
#         assert 'health_check_results' in report
#         assert 'recent_failures' in report
#         assert 'recent_recoveries' in report
#         assert 'suggested_action' in report
    
#     def test_failure_type_determination(self):
#         assert self.monitor._determine_failure_type('network', '') == FailureType.NETWORK
#         assert self.monitor._determine_failure_type('validation_service', '') == FailureType.VALIDATION_SERVICE
#         assert self.monitor._determine_failure_type('database', '') == FailureType.DATABASE
#         assert self.monitor._determine_failure_type('unknown', '') == FailureType.UNKNOWN
